...
#        self.uid = random.randint(0, 100)
    ...
    def update(self, y0, y1):
        ...
        logger.info('Update times: vect %5.3f scal %5.3f seconds' % 
                    (delta[0], delta[1]))
        self.plot_esim_score()
        
        
    def plot_esim_score(self):
        x_val = range(-(self.area[1] / 2), self.area[1] / 2 + 1)
        y_val = range(-(self.area[0] / 2), self.area[0] / 2 + 1)
#        pdb.set_trace()
        X, Y = np.meshgrid(x_val, y_val)
#        pdb.set_trace()
#        Z = self.neig_esim_score[3000].reshape((self.area[1], self.area[0]))
        Z = self.neig_esim_score[3000].reshape(self.area) / self.num_samples
        L = np.linspace(0, np.max(self.neig_esim_score[3000] / self.num_samples), 25)
        # Confourf plot
        fig = pylab.figure()
...
#        ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap=cm.jet, linewidth=0, antialiased=False)
        ...
        if not os.path.exists(folder):
            os.mkdir(folder)
        subfolder = 'peak_anim/' + str(self.uid)
        if not os.path.exists(subfolder):
            os.mkdir(subfolder)
        pylab.savefig('peak_anim/' + str(self.uid) + '/peak_' + str(self.num_samples))
